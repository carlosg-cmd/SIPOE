...
export default function Seguimientos() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        await saveSmartly('seguimientos', 'update', {
          id: editingId,
          ...formData
        });
      } else {
        await saveSmartly('seguimientos', 'insert', {
  ...
  const eliminarSeguimiento = async (id) => {
    ...
    try {
      await saveSmartly('seguimientos', 'delete', { id });
      fetchSeguimientos();
    } catch (err) {
  ...
  const eliminarTodosSeguimientos = async (estudiante_id) => {
    ...
    try {
      const segsToDelete = seguimientos.filter(s => s.estudiante_id === estudiante_id);
      for (const seg of segsToDelete) {
        await saveSmartly('seguimientos', 'delete', { id: seg.id });
      }
      fetchSeguimientos();
    } catch (err) {
      alert("Error al eliminar todos los seguimientos: " + err.message);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto h-full w-full flex flex-col">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 flex-shrink-0">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">Control de Seguimientos</h1>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 mb-6 shadow-sm flex items-center flex-shrink-0">
