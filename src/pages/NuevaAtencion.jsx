...
export default function NuevaAtencion() {
  ...
  async function fetchEstudiantes() {
    ...
    setListaEstudiantes(allData);
  };

  useEffect(() => {
    localStorage.removeItem('atencionDraft'); // Limpiar borradores viejos
    fetchEstudiantes();
    if (editIdUrl) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditing) {
        ...
        atencionIdToUse = crypto.randomUUID();
    ...
    if (formData.activacion_ruta && formData.activacion_ruta !== 'Ninguna') {
      try {
        ...
        console.error("Error guardando ruta activa:", rutaError);
      }
    }
    ...
    if (est && (formData.director_grupo !== est.director_grupo || formData.telefono !== est.telefono)) {
      try {
        await saveSmartly('estudiantes', 'update', {
          id: formData.estudiante_id,
          director_grupo: formData.director_grupo || null,
          telefono: formData.telefono || null
        });
      } catch (estError) {
        console.error("Error actualizando estudiante:", estError);
      }
    }
  ...
  return (
    <div className="max-w-4xl mx-auto h-full w-full flex flex-col">
      ...
      <div className="flex items-center mb-6 flex-shrink-0">
        <Link to="/atenciones" className="p-2 mr-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">{isEditing ? 'Editar Atención' : 'Registrar Nueva Atención'}</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{isEditing ? 'Modifica los datos del registro existente.' : 'Completa los campos para generar el registro en el historial.'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
          <h2 className="text-base font-bold text-slate-800 dark:text-white mb-4 border-b border-slate-100 dark:border-slate-700 pb-2">1. Datos Básicos</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              <label className="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Estudiante a atender</label>
              <Select
                options={listaEstudiantes.map(e => ({ value: e.id, label: `${e.nombres || ''} ${e.apellidos || ''} - ${e.grado || ''}`.trim() }))}
                value={formData.estudiante_id ? { 
                  value: formData.estudiante_id, 
                  label: listaEstudiantes.find(e => e.id === formData.estudiante_id) 
                    ? `${listaEstudiantes.find(e => e.id === formData.estudiante_id).nombres || ''} ${listaEstudiantes.find(e => e.id === formData.estudiante_id).apellidos || ''} - ${listaEstudiantes.find(e => e.id === formData.estudiante_id).grado || ''}`.trim() 
