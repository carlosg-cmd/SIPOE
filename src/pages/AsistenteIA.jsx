    ...
    <div className="max-w-7xl mx-auto h-full w-full flex flex-col min-h-0">
      <div className="mb-2 flex-shrink-0">
        <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white flex items-center">
          <Bot className="w-6 h-6 mr-3 text-indigo-600 dark:text-indigo-400" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 flex-1 overflow-hidden min-h-0">
        ...
        <div className="lg:col-span-1 space-y-4">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white mb-4">Plataformas Recomendadas</h2>
          {IAs.map((ia) => (
            <a
              key={ia.nombre}
              href={ia.url}
              ...
              className="block bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 hover:border-indigo-300 dark:hover:border-indigo-500 hover:shadow-md transition-all group"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center">
                  ...
                  <h3 className="font-bold text-slate-900 dark:text-white text-lg">{ia.nombre}</h3>
                </div>
                <ExternalLink className="w-5 h-5 text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors" />
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400">{ia.desc}</p>
            </a>
          ))}
        ...
        <div className="lg:col-span-2 flex flex-col min-h-0">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-4 sm:p-6 shadow-sm flex-1 flex flex-col min-h-0">
            ...
            <div className="space-y-4 flex-1 flex flex-col min-h-0">
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">¿En qué necesitas ayuda hoy?</label>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <button 
                    onClick={() => setTipoPrompt('remision')}
                    className={`px-4 py-3 text-sm font-medium rounded-xl border text-left transition-all ${tipoPrompt === 'remision' ? 'border-indigo-600 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 ring-1 ring-indigo-600' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                  >
                    <BookOpen className="w-4 h-4 mb-2 text-indigo-500" />
                    Redactar Remisión
                  </button>
                  <button 
                    onClick={() => setTipoPrompt('seguimiento')}
                    className={`px-4 py-3 text-sm font-medium rounded-xl border text-left transition-all ${tipoPrompt === 'seguimiento' ? 'border-emerald-600 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 ring-1 ring-emerald-600' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                  >
                    <MessageSquare className="w-4 h-4 mb-2 text-emerald-500" />
                    Acta de Seguimiento
                  </button>
                  <button 
                    onClick={() => setTipoPrompt('leyes')}
                    className={`px-4 py-3 text-sm font-medium rounded-xl border text-left transition-all ${tipoPrompt === 'leyes' ? 'border-amber-600 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 ring-1 ring-amber-600' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700'}`}
                  >
                    <Scale className="w-4 h-4 mb-2 text-amber-500" />
              ...
              <div className="flex-1 flex flex-col min-h-0">
                ...
                <textarea
                  ...
                  className="flex-1 min-h-0 w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-slate-800 dark:text-slate-200 resize-none transition-shadow text-sm"
                ></textarea>
              </div>

              <button
                onClick={generarPrompt}
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition-colors flex justify-center items-center text-sm flex-shrink-0"
              >
              ...
              {generatedPrompt && (
                <div className="mt-4 animate-fade-in-up flex-1 flex flex-col min-h-0">
                  <div className="flex justify-between items-center mb-2 flex-shrink-0">
                    <label className="block text-sm font-bold text-indigo-700 dark:text-indigo-400">Tu texto listo para la IA:</label>
                    <button 
                      onClick={copiarPrompt}
                      className="flex items-center text-sm font-bold text-indigo-600 hover:text-indigo-800 bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1.5 rounded-lg transition-colors"
                    >
                  ...
                  <div className="relative flex-1 flex flex-col min-h-0">
                    <textarea
                      ...
                      className="flex-1 min-h-0 w-full p-3 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-xl text-slate-700 dark:text-slate-200 text-sm shadow-inner resize-none"
                    />
                  </div>
                </div>
              )}
