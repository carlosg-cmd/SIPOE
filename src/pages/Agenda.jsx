...
export default function Agenda() {
  ...
  const handleSubmit = async (e) => {
    ...
    let error;
    try {
      await saveSmartly('agenda', 'insert', {
  ...
  const marcarCompletado = async (id, estadoActual) => {
    ...
    try {
      await saveSmartly('agenda', 'update', { id, estado: nuevoEstado });
      fetchCitas();
    } catch (err) {
      toast.error("Error al marcar como completada: " + err.message);
    }
  };
  ...
  const eliminarCita = async (id) => {
    ...
    try {
      await saveSmartly('agenda', 'delete', { id });
      toast.success("Cita eliminada");
      fetchCitas();
    } catch (err) {
      toast.error("Error al eliminar la cita: " + err.message);
    }
  };
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-xl max-w-lg w-full p-6 sm:p-8 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <Select
                  options={estudiantes.map(e => ({ value: e.id, label: `${e.nombres || ''} ${e.apellidos || ''} ${e.grado ? `- ${e.grado}` : ''}`.trim() }))}
                  value={formData.estudiante_id ? {
                    value: formData.estudiante_id,
                    label: estudiantes.find(e => e.id === formData.estudiante_id)
                      ? `${estudiantes.find(e => e.id === formData.estudiante_id).nombres || ''} ${estudiantes.find(e => e.id === formData.estudiante_id).apellidos || ''}`.trim()
